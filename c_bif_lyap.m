%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Bifurcation and lyapunov exponent diagrams for parameter c_X.
% 
% Adjust the indices listed in lines 24 and 25 to change with c_X.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
nparam = 500;
parameter_vec = linspace(0.0001,1,nparam);
...
param_idx1 = 2;   % use c1 in LPA model
param_idx2 = 2;   % use c1 in LPAA model
...
for i=1:length(parameter_vec)
    ...
    param_list(param_idx1) = param_temp;
...
parfor(j = 1:length(parameter_vec),8)
    ...
    lyap_params(param_idx1) = parameter_vec(j);
...
for i=1:length(parameter_vec)
    ...
    param_list(param_idx2) = param_temp;
...
parfor(j = 1:length(parameter_vec),8)
    disp("j = "+j)
    lyap_params = param_list;
    lyap_params(param_idx2) = parameter_vec(j);
...
figure()
    ...
    xlabel("$c_1$ (Adult cannibalism on eggs)",'interpreter','latex',"FontSize",24)
    ...
    yline(0,'r:','LineWidth',2)
    xlabel("$c_1$ (Mature cannibalism on eggs) ",'interpreter','latex',"FontSize",24)
    ylabel("Lyapunov exponent",'FontName','Times New Roman')
    ylim([-0.3 0.01])
    xlim([0,1])
    set(gca,'FontName','Times New Roman')
